...
function getActionProof(block_to_prove, requested_action_receipt_digest) {
    let {
        ...
        action_return_value
    } = getReceiptDigests(block_to_prove, requested_action_receipt_digest);
...
function getActionProof2(block_to_prove, requested_action_receipt_digest) {

    let action_receipt_digests, action_return_value;
    const blockNum = block_to_prove.block_num;

    if (global[blockNum + "transactions"] !== undefined) {
        let actionReceiptDigests = [];
        for (const traces of global[blockNum + "transactions"]) {
            for (const trace of traces.action_traces) {
                //if this is the trace of the action we are trying to prove, assign the action_return_value from trace result
                if (trace.receipt_digest === requested_action_receipt_digest && trace.return_value) {
                    action_return_value = trace.return_value.toString()
                }
                actionReceiptDigests.push(trace.receipt_digest);
            }
        }
        action_receipt_digests = actionReceiptDigests;
    } else {
        ({
            action_receipt_digests,
            action_return_value
        } = getShipReceiptDigests(block_to_prove, requested_action_receipt_digest));
    }

    if (global[blockNum + "tree"] === undefined) {
        global[blockNum + "tree"] = createTree(action_receipt_digests);
    }
    if (action_receipt_digests.length > 1) action_receipt_digests = getProof2(global[blockNum + "tree"], requested_action_receipt_digest);
    return {
        amproofpath: action_receipt_digests,
        returnvalue: action_return_value && action_return_value.length ? action_return_value : ""
    };
...
/**
 * 获取action证明
 * @param block_to_prove
 * @returns {{returnvalue: string, amproofpath: *[]}}
 */
function getActionProof3(block_to_prove) {
    const transactions = block_to_prove.transactions;
    if (!transactions) {
        return;
    }
    let actionReceiptDigests = [];
    // for (const traces of transactions) {
    if(transactions.length>0){
        for (const trace of transactions[0].action_traces) {
            const receipt_digest = getReceiptDigest2(trace.receipt);
            actionReceiptDigests.push(receipt_digest);
        }
    }

    // }
    return {
        amproofpath: actionReceiptDigests,
        returnvalue: ""
    };
}

function getActionProofByTree(block_num, requested_action_receipt_digest) {
    const blockTree = block_num + "tree";
    const action_receipt_digests = getProof2(global[blockTree], requested_action_receipt_digest);
    const action_return_value = {};
...
function getShipReceiptDigests(block_to_prove, action_receipt_digest) {
    ...
    const action_receipt_digests = [];
    const transactions = block_to_prove.transactions;
    for (const traces of transactions) {
        for (const trace of traces.action_traces) {
            const receipt_digest = getReceiptDigest2(trace.receipt);
            trace.receipt_digest = receipt_digest
            //if this is the trace of the action we are trying to prove, assign the action_return_value from trace result
            if (receipt_digest === action_receipt_digest && trace.return_value) {
                action_return_value = trace.return_value.toString()
            }
            action_receipt_digests.push(receipt_digest);
        }
    }
    global[block_to_prove.block_num + "transactions"] = transactions;
...
function getProof2(tree, leaf) {

    var proof = [];
    var index = tree.leaves.findIndex(item => item == leaf);

    if (index <= -1) {
        console.log("Couldn't find leaf in tree222222");
        return []
    }
...
function getReceiptDigest1(receipt) {
    const buffer = new SerialBuffer({TextEncoder, TextDecoder});
    ...
    if (receipt.act_digest && !receipt.digest) {
        ...
        for (const auth of receipt.auth_sequence) {
            if (auth[1]) {
                authSequence.push({accountName: auth[0], sequence: auth[1]})
            } else {
                authSequence.push({accountName: auth.account, sequence: auth.sequence})
            } //handle SHIP
        }

        authSequence.sort((a, b) => a.accountName.localeCompare(b.accountName));
        ...
        receipt = {
            ...
            recvSequence: receipt.recv_sequence,
            authSequence,
            codeSequence: receipt.code_sequence || 0,
            abiSequence: receipt.abi_sequence || 0,
        }
    }
    ...
    varuint32.serialize(buffer, receipt.codeSequence);
    varuint32.serialize(buffer, receipt.abiSequence);
 ...
 * @returns {string}
 */
function getReceiptDigest2(receipt) {
    const buffer = new SerialBuffer({TextEncoder, TextDecoder});
    ...
    if (receipt.act_digest && !receipt.digest) {
        auth_sequence = auth_sequence.map(auth => {
            const [accountName, sequence] = auth[1] ? auth : [auth.account, auth.sequence];
            return {accountName, sequence};
        });

        auth_sequence.sort((a, b) => a.accountName.localeCompare(b.accountName));
    }

    name.serialize(buffer, receiver);
    checksum256.serialize(buffer, act_digest);
    uint64.serialize(buffer, global_sequence);
    uint64.serialize(buffer, recv_sequence);
    varuint32.serialize(buffer, auth_sequence ? auth_sequence.length : 0);

    if (auth_sequence) {
        for (const auth of auth_sequence) {
            name.serialize(buffer, auth.accountName);
            uint64.serialize(buffer, auth.sequence);
        }
    }

    varuint32.serialize(buffer, code_sequence);
    varuint32.serialize(buffer, abi_sequence);
